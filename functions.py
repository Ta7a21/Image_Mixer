...
def applyFourier(self, image, compWidget):
    ...
    # Phase = Image.fromarray(np.abs(np.fft.ifft2(phase)), 'RGB')
    # Magnitude = Image.fromarray(np.abs(np.fft.ifft2(magnitude)).astype(np.uint8), 'RGB')
    # Real = Image.fromarray(np.abs(np.fft.ifft2(real)).astype(np.uint8), 'RGB')
    # Phase.save("Phase.jpg")
    # Imaginary.save('Imaginary.jpg')
    # Magnitude.save("Magnitude.jpg")
    # Real.save("Real.jpg")
    # print(np.abs(np.fft.ifft2(phase)))
    plt.imsave ("Phase.png" ,np.abs(np.fft.ifft2(phase)))
    plt.imsave ("Imaginary.png" ,np.abs(np.fft.ifft2(imaginary)).astype(np.uint8))
    plt.imsave ("Magnitude.png" ,np.abs(np.fft.ifft2(magnitude)).astype(np.uint8))
    plt.imsave ("Real.png" ,np.abs(np.fft.ifft2(real)).astype(np.uint8))
    plt.imshow(np.abs(np.fft.ifft2(imaginary)).astype(np.uint8))
    # plt.show()
