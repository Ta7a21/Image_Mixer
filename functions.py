...
class Image:
    def __init__(self, magnitude, phase, real, imaginary, imgSize, pixelSize):
        ...
        self.compnts = {}
        self.uniMagnitude = np.array(
            (magnitude * 0) + 1) if type(real) != int else 0
    ...
    def mixer(self, ratio, compnts, index):
        final_compnts = [0, 0]
        phaseInd = 0
        magInd = 1
...
def browsefiles(self, imageWidget, index):
    ...
    if fname[0].endswith(extensionsToCheck):
        ...
        logging.warning(
            f"The user did not select a valid file format. {fname[0]}")
        return
    else:
        return
...
def read_image(self, filename, imageWidget, index):
    ...
    logging.info(
        f"image{[index+1]} is of size ({img.size[0]},{img.size[1]}) and an alpha of length {pixelSize}.")
    for i in range(2):
        if (
            index == i
            ...
            and (
                imgSize != images[1 -
                                  i].imgSize or pixelSize != images[1 - i].pixelSize
            )
        ):
            errorMssg(self, "Select two images with the same size.")
            logging.critical(
                "User selected images with different sizes or alpha values.")
            return
    ...
    setImage(pixelMap, imageWidget)


def fftComponent(window, comboBox, compntWidget, index):
    ...
    pixelMap = QPixmap.fromImage(qtImage)
    setImage(pixelMap, compntWidget)


def setImage(pixelMap, widget):
...
def output(self):
    ...
    mixerCompnts = [self.mixerComp_1.currentText(),
    ...
    pixelMap = final_img.mixer(sliders,  mixerCompnts, mixerImages)
    setImage(pixelMap, outputWidget)
...
def fftCompConnect(self, comboBox, compntWidget, index):
    comboBox.activated[str].connect(
        lambda: fftComponent(self, comboBox, compntWidget, index)
    )
